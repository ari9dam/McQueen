...
class RoBertaMCQParallelScoreReader:
    ...
    def text_to_instance(self,  # type: ignore
        ...
        for premise, hypothesis in zip(premises, choices):
            ...
            for tup in premise[0:max_number_premises]:
                sentence = tup[0]
                ir_score = tup[1]
                if question is None:
                ...
                per_choices_scores.append(float(ir_score))
            ...
            scores.append(per_choices_scores)

        return (tokens, token_type_ids,scores)

    def read(self, file_path: str, tokenizer, max_seq_len: int, max_number_premises:int=None):
        file_name = file_path.split("/")[-1]
        dir_name = os.path.dirname(file_path)
        cache_file_path =  os.path.join(dir_name, 'cached_roberta_score_{}_{}_{}'.format(file_name,max_seq_len,max_number_premises))
        if os.path.exists(cache_file_path):
            ...
            all_scores = features['scores']
            all_labels = features.get('labels',None)
            if all_labels is not None:
                return TensorDataset(all_tokens, all_segment_ids, all_masks,all_scores, all_labels)
            else:
                return TensorDataset(all_tokens, all_segment_ids, all_masks,all_scores)
        ...
        with open(file_path, 'r') as te_file:
            ...
            for line in tqdm(te_file,desc="preparing dataset:"):
                ...
                pp_tokens, pp_segment_ids, pp_scores = self.text_to_instance(tokenizer, max_seq_len, premises, choices,
